...
EVENTS = {'run_started':'Run started','model_request_started':'Model request started','provider_failure':'Provider failure','backend_failure':'Backend failure','model_response_received':'Model response received','tool_call_started':'Tool call started','tool_call_completed':'Tool call completed','tool_call_failed':'Tool call failed','investigation_submitted':'Investigation submitted','classification_submitted':'Classification submitted','plan_submitted':'Plan submitted','decomposition_submitted':'Decomposition submitted','decomposition_validation_completed':'Decomposition validation completed','execution_path_selected':'Execution path selected','candidate_attempt_started':'Candidate attempt started','candidate_attempt_completed':'Candidate attempt completed','candidate_attempt_failed':'Candidate attempt failed','candidate_started':'Candidate started','candidate_completed':'Candidate completed','candidate_failed':'Candidate failed','runner_started':'Runner started','runner_completed':'Runner completed','runner_failed':'Runner failed','review_started':'Review started','review_completed':'Review completed','review_failed':'Review failed','selection_started':'Selection started','selection_completed':'Selection completed','winner_selected':'Winner selected','final_decision':'Final decision','subtask_attempt_started':'Subtask attempt started','subtask_attempt_completed':'Subtask attempt completed','subtask_attempt_failed':'Subtask attempt failed','subtask_attempt_reviewed':'Subtask attempt reviewed','subtask_accepted':'Subtask accepted','subtask_failed':'Subtask failed','validation_started':'Validation started','validation_completed':'Validation completed','validation_failed':'Validation failed','candidate_attempt_reviewed':'Candidate attempt reviewed','selection_completed':'Selection completed','run_finalized':'Final decision','decomposition_deadlock_detected':'Decomposition deadlock detected','candidate_fallback_started':'Candidate fallback started','integration_started':'Integration started','integration_completed':'Integration completed','integration_failed':'Integration failed','recovery_injected':'Recovery injected','recovery_deterministic_action_executed':'Recovery action executed'}
STATUS_BY_EVENT = {'run_started':'running','candidate_attempt_started':'running','subtask_attempt_started':'running','validation_started':'running','runner_started':'running','review_started':'running','selection_started':'running','integration_started':'running','model_request_started':'running','tool_call_started':'running','candidate_fallback_started':'running','candidate_attempt_completed':'completed','candidate_completed':'completed','runner_completed':'completed','review_completed':'completed','subtask_attempt_completed':'completed','validation_completed':'completed','integration_completed':'completed','candidate_attempt_failed':'failed','candidate_failed':'failed','runner_failed':'failed','review_failed':'failed','validation_failed':'failed','subtask_attempt_failed':'failed','subtask_failed':'failed','validation_failed':'failed','integration_failed':'failed','subtask_accepted':'accepted','candidate_attempt_reviewed':'completed','subtask_attempt_reviewed':'completed','selection_started':'running','selection_completed':'selected','winner_selected':'selected','final_decision':'completed','run_finalized':'completed','provider_failure':'failed','backend_failure':'failed','tool_call_failed':'failed','model_response_received':'completed','tool_call_completed':'completed','decomposition_deadlock_detected':'blocked'}
...
def _subtitle(ev: dict[str,Any]) -> str:
    ...
    elif 'candidate' in t or t.startswith('validation_') or t.startswith('review_') or t.startswith('runner_') or t in {'selection_started','selection_completed','winner_selected','final_decision'}:
        bits=[humanize_id(_attempt_id(ev) or '')]
        for k in ('status','result','decision','failure_message','warning','message'):
            if p.get(k): bits.append(human_label(p.get(k)) if k in {'status','result','decision'} else p.get(k))
...
def build_viewer_graph_layout(snapshot_or_state: dict[str,Any], events: list[dict[str,Any]]) -> dict[str,Any]:
    ...
    if pf_kind and not _candidate_execution_happened(state, events):
        ...
        nodes=[
            ...
            {'id':'provider_failure','label':'Provider failure','type':'failure','row':1,'col':3,'status':'failed','subtitle':human_label(pf_kind),'summary':human_label(pf_kind),'details':{'node_type':'Provider failure','failure_kind':pf_kind,'failure_label':human_label(pf_kind),'failure_message':msg,'backend_name':backend_name,'model':_model(state, [], events),'backend_url':backend_url,'recoverable':state.get('recoverable', p.get('recoverable')),'next_step':'Start the backend server or update backend configuration.'}},
    ...
    if candidates:
        ...
        for i,c in enumerate(candidates,1):
            ...
            detail={'candidate_id':cid,'stage':'Candidate','status':human_label(c.get('status') or 'completed'),'patch_evidence':norm.patch,'changed_files':changed,'changed_files_count':len(changed),'runner_status':human_label(c.get('runner_status') or c.get('status') or 'unknown'),'review_status':human_label(c.get('review_status') or (c.get('review') or {}).get('decision') or 'unknown'),'validation_status':human_label(_validation_status(c)),'warnings':c.get('warnings') or [],'blockers':c.get('acceptance_blockers') or [],'winner_selected':cid==sel,'eligibility':c.get('acceptance_eligible'),'raw':c}
            ...
            add(f'{cid}_runner','Runner '+human_label(rstat).lower(),'runner',i,2,'failed' if str(rstat).lower() in {'failed','error'} else 'completed',f"exit {c.get('exit_code')}" if c.get('exit_code') is not None else 'Runner status',{'candidate_id':cid,'stage':'Runner','status':human_label(rstat),'runner_status':human_label(rstat),'patch_evidence':norm.patch,'changed_files':changed,'raw':c.get('runner') or c})
            ...
            add(f'{cid}_review',rev_label,'review',i,3,'failed' if 'failed' in rev_label.lower() else 'completed',human_label(rev),{'candidate_id':cid,'stage':'Review','status':human_label(rev),'review_status':human_label(rev),'patch_evidence':norm.patch,'changed_files':changed,'warnings':c.get('warnings') or [],'blockers':c.get('acceptance_blockers') or [],'raw':c.get('review') or rev},'Failed' if 'failed' in rev_label.lower() else None)
            ...
            add(f'{cid}_validation',vlabel,'validation',i,4,vst,('Warning: validation not run' if vst=='missing' else human_label(val)),{'candidate_id':cid,'stage':'Validation','status':human_label(val),'validation_status':human_label(val),'patch_evidence':norm.patch,'changed_files':changed,'warnings':['Validation did not run'] if vst=='missing' else [],'blockers':c.get('acceptance_blockers') or [],'winner_selected':cid==sel,'raw':c.get('validation') or val},badge)
