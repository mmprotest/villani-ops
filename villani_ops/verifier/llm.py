...
For the final verdict, explicitly identify the most critical stated or implied requirement, the strongest direct evidence for it, whether that evidence actually covers the requirement rather than a weaker nearby condition, and whether the run should be automatically accepted. Do not mark criticalRequirementCovered true from import checks alone, source plausibility alone for behavioural tasks, comments or stated intent, normal-path validation for abnormal-path requirements, or candidate-authored validation that tests only a weaker condition. Mark it true only when evidence directly exercises, demonstrates, or proves the critical requirement.
