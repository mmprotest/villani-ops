def render(result):
    ...
    lines += [f"Result: {result.get('result')}", f"Verdict: {result['verdict']}",f"Confidence: {float(result.get('confidence',0)):.2f}",f"Recommended action: {result['recommendedAction']}",'','Reason:',str(result.get('reason',''))]
    return '\n'.join(lines)+'\n'
def exit_code(v): return {'success':0,'failure':1,'error':3}.get(v,3)
