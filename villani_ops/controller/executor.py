    ...
    def run(self, repo: str|Path, task: Task, candidate_attempts: int=3, timeout_seconds: int|None=None, classify: bool=True, non_interactive: bool=False, isolation: str='worktree') -> RunResult:
        ...
        for idx,b in enumerate(coding_plan, start=1):
            ...
            try:
                ...
                res=runner.run(RunnerContext(attempt_id=aid, repo_path=wt['worktree_path'], task_instruction=_candidate_prompt(task, inv, idx, candidate_attempts), success_criteria=task.success_criteria, backend=b, timeout_seconds=timeout_seconds or b.timeout_seconds or 1200, run_dir=str(adir)))
