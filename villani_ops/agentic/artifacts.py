...
def derive_graph(state, events: list[dict]) -> dict:
    ...
    for i in range(1,((getattr(state,'candidate_concurrency',{}) or {}).get('batch_count') or 0)+1): nodes.append({'id':f'candidate_batch_{i}','type':'candidate_batch','max_parallel':getattr(state,'max_parallel',None)})
