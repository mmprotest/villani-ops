...
def derive_graph(state, events:list[dict])->dict:
    nodes=[{'id':'run','type':'run','status':state.status,'concurrency_mode':getattr(state,'concurrency_mode',None),'max_parallel':getattr(state,'max_parallel',None)},{'id':'investigation','type':'investigation','present':state.investigation is not None},{'id':'plan','type':'plan','present':state.plan is not None}]
