...
def derive_graph(state, events:list[dict])->dict:
    ...
    if state.integration: nodes.append({'id':'integration','type':'integration','status':state.integration.get('status'),'failure_reason':state.integration.get('failure_reason')})
