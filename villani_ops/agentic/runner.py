...
class AgenticLLMReviewer:
    ...
    def review(self, *, state, attempt, scope):
        ...
        messages=[{'role':'user','content':'Review this attempt using the forced structured review tool. Put real correctness, scope, runtime, security, and patch-hygiene blockers in blockers. Put uncertainty about skipped validation, no reliable validation command, inconclusive validation, unverified status, or inability to confirm an official verifier in issues/uncertainty/warnings, not blockers. If the patch looks plausible but unverified, decision may be pass and recommended_action accept while validation uncertainty remains recorded separately. Do not claim verified success without reliable validation, and do not make missing authoritative validation a patch-quality failure by itself.\n'+__import__('json').dumps(payload,indent=2,default=str)[:120000]}]
        resp=self.client.create_message(backend=self.review_backend,messages=messages,system='You are a strict production code reviewer for Villani Ops agentic mode. Review patch quality, scope, runner evidence, and validation evidence separately. Treat validation infrastructure_error as a command that failed to test the candidate, not proof the candidate is wrong. diagnostic_failed and generated smoke checks are weak evidence only; passing import/signature/file-exists checks is not proof of full success. Do not claim verified success without reliable explicit or high-confidence project validation. Lack of authoritative validation belongs to the central acceptance gate: record uncertainty in issues, warnings, non_blocking_issues, uncertainty, or validation_uncertainty, but do not add validation_missing/no reliable command/inconclusive validation as a hard review blocker when the patch is otherwise plausible. Allow plausible best-candidate selection as unverified when no reliable validation is available, and distinguish patch-quality rejection from validation-unverified status.',tools=[tool],tool_choice={'type':'function','function':{'name':'agentic_review_decision'}},strict=True)
