...
class CandidateAttemptState(BaseModel):
    ...
    oracle_coverage_score:float=0.0; critical_requirements_failed:list[str]=Field(default_factory=list); critical_requirements_uncertain:list[str]=Field(default_factory=list); probes_passed:list[str]=Field(default_factory=list); probes_failed:list[str]=Field(default_factory=list)
...
class OpsRunState(BaseModel):
    ...
    reviews:list[dict]=Field(default_factory=list); risk_reviews:dict[str,dict]=Field(default_factory=dict); pairwise_comparisons:list[dict]=Field(default_factory=list); tournament_ranking:dict|None=None; candidate_agreement_summary:dict|None=None; repo_validation_results:list[dict]=Field(default_factory=list); selection:dict|None=None; final_decision:dict|None=None
    ...
    warnings:list[str]=Field(default_factory=list); blockers:list[str]=Field(default_factory=list); reserve_finalization_seconds:int=60; reserve_review_seconds:int=120; max_review_retries:int=2; max_malformed_review_retries:int=2; candidate_generation_deadline:float|None=None; candidate_attempts_launched:int|None=None; candidate_attempt_launch_reason:str|None=None; concurrency_mode:str|None=None; max_parallel:int|None=None; execution_concurrency:dict=Field(default_factory=dict); candidate_concurrency:dict=Field(default_factory=dict); subtask_concurrency:dict=Field(default_factory=dict); batch_count:int|None=None; wave_count:int|None=None; recovery_count:int=0; last_error:str|None=None; last_tool_name:str|None=None; last_tool_input:dict|None=None
