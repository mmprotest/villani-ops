...
class Selector:
    ...
    def select(self, task: Any, investigation: Any, candidates: list[dict[str, Any]], backend_name: str, backend: Backend, run_dir: str|Path) -> tuple[SelectionResult, LLMCallResult|None]:
        ...
        ctx={"task": task.model_dump(mode='json'), "performance_backend": {"name": backend_name, "model": backend.model}, "investigation": investigation.model_dump(mode='json') if investigation else None, "candidates": candidates}
        (run_dir/'selection_input.json').write_text(json.dumps(ctx, indent=2))
        if not any(c.get('acceptance_eligible') for c in candidates):
            sel=deterministic_fallback(candidates); sel.selector_backend=backend_name; sel.performance_backend={'name': backend_name, 'model': backend.model}
        ...
        try:
            ...
            sel=SelectionResult.model_validate(call.parsed_json); sel.selector_backend=backend_name; sel.performance_backend={'name': backend_name, 'model': backend.model}
            ...
            sel=deterministic_fallback(candidates); sel.selector_backend=backend_name; sel.performance_backend={'name': backend_name, 'model': backend.model}; call=locals().get('call')
