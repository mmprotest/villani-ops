...
def write_performance_report(run_dir: str|Path, task: Any, investigation: Any, candidates: list[dict], selection: Any, decision: Any, duration: float, mode: str = 'performance', runner: str = 'villani-code', graph: Any = None, selected_backend_per_node: dict | None = None, routing_decisions: dict | None = None) -> Path:
    ...
    if investigation:
        lines += [f"Summary: {investigation.summary}", f"Suspected root cause: {investigation.suspected_root_cause or ''}", 'Relevant files: '+', '.join(investigation.relevant_files), 'Relevant tests: '+', '.join(investigation.relevant_tests), f"Investigation normalized: {str(getattr(investigation, 'investigation_normalized', False)).lower()}", 'Investigation normalization notes:', *[f"- {x}" for x in getattr(investigation, 'investigation_normalization_notes', [])], f"Investigation fallback used: {str(getattr(investigation, 'investigation_fallback_used', False)).lower()}", f"Investigation fallback reason: {getattr(investigation, 'investigation_fallback_reason', '') or ''}", 'Risks:', *[f"- {x}" for x in investigation.risks], f"Confidence: {investigation.confidence}"]
    plan=decision.plan or {}; dec=decision.decomposition or {}
    lines += ['', '## Plan', f"Strategy: {plan.get('strategy','')}", f"Should decompose: {plan.get('should_decompose', False)}", f"Candidate attempts: {plan.get('candidate_attempts', decision.candidate_attempts_requested)}", f"Difficulty: {plan.get('expected_difficulty','')}", f"Planner normalized: {str(plan.get('planner_normalized', False)).lower()}", f"Planner fallback used: {str(plan.get('planner_fallback_used', plan.get('fallback_used', False))).lower()}", f"Planner fallback reason: {plan.get('planner_fallback_reason', '') or ''}", 'Planner normalization notes:', *[f'- {x}' for x in plan.get('planner_normalization_notes', [])], 'Risks:', *[f"- {x}" for x in plan.get('risks', [])], '', '## Decomposition']
    if dec:
