...
def write_performance_report(run_dir: str|Path, task: Any, investigation: Any, candidates: list[dict], selection: Any, decision: Any, duration: float) -> Path:
    ...
    lines=['# Villani Ops Performance Run Report','', '## Task', f"Objective: {task.objective or task.instruction or ''}", f"Success criteria: {task.success_criteria or ''}", '', '## Mode', 'performance_orchestration', '', '## Classification', 'See classification.json if present.', '', '## Investigation']
    if investigation:
    ...
    lines += ['', '## Final Decision', f"Accepted: {decision.accepted}", f"Winner: {decision.winning_attempt_id or ''}", f"Failure reason: {decision.failure_reason}", 'Acceptance blockers:', *[f"- {x}" for x in decision.acceptance_blockers], '', '## Telemetry', f"Input tokens: {decision.total_input_tokens}", f"Output tokens: {decision.total_output_tokens}", f"Coding cost: {decision.coding_cost}", f"Review cost: {decision.review_cost}", f"Costs are telemetry only; main run does not optimize for cost.", f"Duration seconds: {duration:.1f}", '', '## Next Commands']
