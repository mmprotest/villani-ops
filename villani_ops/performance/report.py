...
def _selection_evidence_lines(candidates: list[dict], selection: Any) -> list[str]:
    ...
    lines += [f"Fallback used: {getattr(selection, 'fallback_used', False) if selection else False}", f"Selector backend: {getattr(selection, 'selector_backend', None) or ''}", f"Selector confidence: {getattr(selection, 'confidence', '') if selection else ''}"]
    return lines
